            ...
            if (ImGui::BeginTabItem("Sabotage")) {
                ...
                if (ImGui::Button("Sabotage All")) {
                    ...
                    if (State.mapType == Settings::MapType::Ship || State.mapType == Settings::MapType::Hq || State.mapType == Settings::MapType::Fungle)
                        State.rpcQueue.push(new RpcRepairSystem(SystemTypes__Enum::Reactor, 128));
                    else if (State.mapType == Settings::MapType::Pb)
                        State.rpcQueue.push(new RpcRepairSystem(SystemTypes__Enum::Laboratory, 128));
                    else if (State.mapType == Settings::MapType::Airship)
                        State.rpcQueue.push(new RpcRepairSystem(SystemTypes__Enum::Reactor, 128));

                    if (State.mapType == Settings::MapType::Ship || State.mapType == Settings::MapType::Hq)
                        State.rpcQueue.push(new RpcRepairSystem(SystemTypes__Enum::LifeSupp, 128));

                    if (State.mapType == Settings::MapType::Fungle)
                        State.rpcQueue.push(new RpcRepairSystem(SystemTypes__Enum::Sabotage, SystemTypes__Enum::MushroomMixupSabotage));

                    State.rpcQueue.push(new RpcRepairSystem(SystemTypes__Enum::Comms, 128));
                }
